...
describe Admin::FosterAnimalsController do
    ...
    describe 'with valid params' do
      it 'creates a new foster_animal' do
        FosterAnimal.observers.disable :all do
          expect do
            post :create, params: { foster_animal: { animal_id: @animal.id, adoption_contact_id: @foster_contact.id } }
          end.to change(FosterAnimal, :count).by(1)
        end
      end
  ...
  describe 'DELETE destroy' do
    it 'destroys the requested foster_animal' do
      expect do
        delete :destroy, params: { id: @foster_animal.animal.id, foster: @foster_animal.foster_contact.id }
      end.to change(FosterAnimal, :count).by(-1)
    end
