...
feature 'Adoption Contacts' do 
  ...
  context 'creating an adoption contact' do
    scenario 'user creates an adoption contact' do
      ...
      click_on 'Add New Adoption Contact'
      ...
      expect(page).to have_content 'Adoption Contact was successfully created.'
    end
  end

  context 'editing/viewing an adoption contact', js: true do
    before do
      contact = FactoryGirl.create(:adoption_contact,
      ...
      sleep 0.2
      visit admin_adoption_contact_path(contact.id)
    end

    scenario 'user edits an existing adoption contact' do
      # changing the name
    ...
    scenario 'user can add and remove animals to an adoption contact' do
      # add
      within '.nav-tabs' do
        click_on 'Animals'
      end
      click_on 'Add Animal'
      click_on 'Submit'
      expect(page).to have_content 'Adopt Animal was successfully created.'
      within '.nav-tabs' do
        click_on 'Animals'
      end
  ...
  context 'listing adoption contacts' do
    before do
      @contact = FactoryGirl.create(:adoption_contact,
                                    organization_id: @user.organization_id)
      visit admin_adoption_contacts_path
    ...
    scenario 'user views a list of adoption contacts' do
      within 'table' do
    ...
    scenario 'user can sort the table', js: true do
      second_contact = FactoryGirl.create(:adoption_contact,
                                          organization_id: @user.organization_id,
                                          first_name: 'zelda',
                                          email: 'a@example.com')
      visit admin_adoption_contacts_path
    ...
    scenario 'user downloads a csv of the adoption contacts' do
      require 'csv'
      click_on 'Export'
    ...
    scenario 'user deletes an adoption contact', js: true do
      ...
      expect(page).to have_content 'Adoption Contact was successfully deleted.'
    ...
    scenario 'uses pagination' do
      15.times { FactoryGirl.create(:adoption_contact, organization_id: @user.organization_id, first_name: 'yolo') }
      visit admin_adoption_contacts_path
      within first('tbody') do
        expect(page).to_not have_content @contact.first_name
      end
