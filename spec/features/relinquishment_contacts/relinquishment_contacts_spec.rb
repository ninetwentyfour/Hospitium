...
feature 'Relinquishment Contacts' do 
  before :each do
    @user = new_logged_in_user
  end
 
  context 'creating an relinquishment contact' do
    scenario 'user creates an relinquishment contact' do
      visit admin_relinquishment_contacts_path
      click_on 'Add New Relinquishment Contact'
      ...
      expect(page).to have_content 'Relinquishment Contact was successfully created.'
    end
  end

  context 'editing/viewing an relinquishment contact', js: true do
    before do
      contact = FactoryGirl.create(:relinquishment_contact,
      ...
      sleep 0.2
      visit admin_relinquishment_contact_path(contact.id)
    end

    scenario 'user edits an existing relinquishment contact' do
      # changing the name
    ...
    scenario 'user can add and remove animals to an relinquishment contact' do
      # add
      within '.nav-tabs' do
        click_on 'Animals'
      end
      click_on 'Add Animal'
      click_on 'Submit'
      expect(page).to have_content 'Animal successfully added.'
      within '.nav-tabs' do
        click_on 'Animals'
      end
  ...
  context 'listing relinquishment contacts' do
    before do
      @contact = FactoryGirl.create(:relinquishment_contact,
                                    organization_id: @user.organization_id)
      visit admin_relinquishment_contacts_path
    ...
    scenario 'user views a list of relinquishment contacts' do
      within 'table' do
    ...
    scenario 'user can sort the table', js: true do
      second_contact = FactoryGirl.create(:relinquishment_contact,
                                          organization_id: @user.organization_id,
                                          first_name: 'zelda',
                                          email: 'a@example.com')
      visit admin_relinquishment_contacts_path
    ...
    scenario 'user downloads a csv of the relinquishment contacts' do
      require 'csv'
      click_on 'Export'
      expect(page.response_headers['Content-Type']).to eq 'text/csv'
      csv = CSV.parse(page.body)

      expect(csv.first).to eq ['ID', 'First Name', 'Last Name', 'Address', 'Phone', 'Email', 'Reason', 'Relinquished Animal IDs']
    ...
    scenario 'user deletes an relinquishment contact', js: true do
      within first('tbody tr') do
        expect(page).to have_content @contact.first_name
        expect(page).to have_content @contact.last_name
      ...
      expect(page).to have_content 'Relinquishment Contact was successfully deleted.'
    ...
    scenario 'uses pagination' do
      15.times { FactoryGirl.create(:relinquishment_contact, organization_id: @user.organization_id, first_name: 'yolo') }
      visit admin_relinquishment_contacts_path
      within first('tbody') do
        expect(page).to_not have_content @contact.first_name
      end
