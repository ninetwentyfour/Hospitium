...
#     assign_attributes
#     updated = with_callbacks(:update, :save) { entry.save }
#     respond_with(:admin, entry, options.reverse_merge(:success => updated), &block)
#   end
...
#   # Set a success flash notice when we got a HTML request.
#   def set_success_notice
#     flash[:notice] = "#{action_name.capitalize} on #{models_label(false)} was successful." if request.format == :html
#   end
