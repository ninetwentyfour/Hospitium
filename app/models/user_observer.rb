...
class UserObserver < ActiveRecord::Observer
  ...
  def send_user_confirmed_email(user)
    if Rails.env == "test"
      ...
      url = "https://sendgrid.com/api/mail.send.json?api_user=#{ENV['SENDGRID_USERNAME']}&api_key=#{ENV['SENDGRID_PASSWORD']}&to=contact@travisberry.com&subject=Hospitium%20-%20User%20Confirmed&text=#{URI::encode(user.username)}%20confirmed%20an%20account.%20#{URI::encode(user.email)}%20in%20organization%20#{URI::encode(user.organization_name)}&from=contact@hospitium.co"
